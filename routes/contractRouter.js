...
router.get('/single-contract/:id', async (req, res) => {
    try {
        const contract = await Contract.findById(req.params.id)
            ...
            .populate('products', 'name')
            .populate('created_by', 'name')
            .populate('selected_users', 'name image role') 
            ...
            .populate({ 
                ...
                populate: [
                    ...
                    { path: 'hod_ref', select: 'name  role image' },
                    { path: 'sale_manager_ref', select: 'name  role image' },
                    { path: 'hom_ref', select: 'name  role image' },
